...
class Job:
    ...
    def __str__(self):
        ...
        s += "Micro-tasks:\n"
        metadata = zip(
            self._worker_types, self._start_times, self._end_times, self._allocations
        )
        for i, (worker_type, start_time, end_time, allocation) in enumerate(metadata):
            s += "\t%2d) [%4s] %.3f - %.3f (%.3f) (%s)\n" % (
                i,
                worker_type,
                start_time,
...
def get_overall_execution_time(jobs, max_end_time=None):
    ...
    for job_id in jobs:
        ...
        if latest_end_time is None or jobs[job_id].end_times[-1] > latest_end_time:
...
def print_job_summary(jobs, num_jobs):
    ...
    worker_runtimes, restricted_execution_time = get_worker_runtimes(jobs, num_jobs)
    ...
    print(
        ...
        % (sum(queueing_delays) / (sum(queueing_delays) + sum(computation_times)))
    )
    ...
    for job_id in jobs:
        ...
        for i, worker_id in enumerate(job.worker_ids):
            ...
            events[worker_id].append(
                (job_id, job.worker_types[i], job.start_times[i], job.end_times[i])
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--log_files", type=str, nargs="+", required=True, help="Log file"
    )
    parser.add_argument(
