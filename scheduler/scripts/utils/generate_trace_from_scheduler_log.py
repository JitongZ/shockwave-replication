    ...
    if args.log is not None:
        ...
        with open(args.log, "r") as f:
            for line in f:
                ...
                match = re.search(
                    "Received lease update request.*" "job_id=(\d+).* steps=(\d+)", line
                )
                if match is not None:
                    job_id = int(match.group(1))
                ...
                if match is not None:
                    for job_id in in_progress_steps:
                        ...
                        assert num_updates == 0 or num_updates == scale_factors[job_id]
    ...
    with open(args.output_trace, "w") as f:
        for job_id, (job, arrival_time) in enumerate(zip(jobs, arrival_times)):
            if job_id not in remaining_steps:
                continue
            job.total_steps = remaining_steps[job_id] - sum(in_progress_steps[job_id])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_trace", type=str, required=True, help="Input trace")
    parser.add_argument(
        "--log", type=str, default=None, help="Execution log before failure"
    )
    parser.add_argument("--output_trace", type=str, required=True, help="Output trace")
