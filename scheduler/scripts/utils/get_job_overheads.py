...
def main(args):
    if not os.path.isdir(args.timeline_dir):
        raise ValueError('Invalid timeline directory "{0}"'.format(args.timeline_dir))
                ...
                for j, worker in enumerate(sorted(os.listdir(job_dir))):
                    ...
                    print(
                        ...
                        "total time={3:.2f}, overhead={4:.2f}%".format(
                            i, j, computation_time, total_time, 100.0 * overhead
                        )
                    )
            print()
...
hiding which we do not include in this analysis."""
    ...
    parser.add_argument(
        "--timeline_dir", required=True, type=str, help="Root timeline directory"
    )
