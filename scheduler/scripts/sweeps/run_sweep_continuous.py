...
def main(args):
    ...
    if args.throughput_lower_bound is None or args.throughput_upper_bound is None:
        ...
        for policy_name in policy_names:
            ...
            for profiling_percentage in profiling_percentages:
                ...
                for i, num_reference_models in enumerate(args.num_reference_models):
                    if estimate_throughputs:
                        num_reference_models_str = "num_reference_models=%d" % (
                            num_reference_models
                        )
                        ...
                        raw_logs_num_reference_models_subdir = raw_logs_policy_subdir
                    throughputs = list(
                        np.linspace(
                    ...
                    for throughput in throughputs:
                        if (
                            ...
                            and policy_name in cutoff_throughputs[cluster_spec_str]
                        ):
                            cutoff_throughput = cutoff_throughputs[cluster_spec_str][
                                policy_name
                            ]
                            if throughput >= cutoff_throughput:
                                print(
                                    ...
                                    "spec %s."
                                    % (throughput, policy_name, cluster_spec_str)
                                )
                                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--interval", type=int, default=360, help="Interval length (in seconds)"
    )
    parser.add_argument(
        "-f",
        "--fixed-job-duration",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[1.0],
        help=("Percentages of machines dedicated to profiling " "co-located job pairs"),
    )
    parser.add_argument(
        "--num_reference_models",
        type=int,
        ...
        help=("Number of reference models to use when " "estimating throughputs"),
    )
    parser.add_argument(
        "--ideal",
        action="store_true",
