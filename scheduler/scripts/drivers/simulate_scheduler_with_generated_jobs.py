...
def simulate(
    ...
    print(
        ...
        "seed=%d, lam=%f" % (current_time, cluster_spec_str, policy.name, seed, lam),
        file=sys.stderr,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--lam", type=float, required=True, help="Lambda for Poisson arrival rate"
    )
    parser.add_argument(
        "-f",
        "--fixed-job-duration",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help=("Percentage of co-located throughput values " "measured for each job"),
    )
    parser.add_argument(
