...
BASE_WORKER_COMMAND = """python worker.py -i 127.0.1.1 -s 50060 -w 50061 -g 1 -t p100"""
...
def main(args):
    ...
    for model, model_dir in models:
        ...
        for num_jobs in [1, 2]:
            ...
            num_jobs_dir = os.path.join(model_dir, "num_jobs={0}".format(num_jobs))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Sweep models to collect job timelines for " "measuring overhead"
    )
    parser.add_argument(
        "--max_rounds", type=int, default=10, help="Maximum number of rounds to run"
    )
    parser.add_argument(
        "--timeline_dir",
        type=str,
    ...
    parser.add_argument(
        "--data_dir", type=str, required=True, help="Directory where data is stored"
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
