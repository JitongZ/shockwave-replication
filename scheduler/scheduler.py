...
class Scheduler:
    ...
    def __init__(
        ...
        self._logger = SchedulerAdapter(
            logger, {"scheduler": self, "start_timestamp": datetime.datetime.now()}
        )
        ...
        # extended leases.
        self._completion_event_scheduler = sched.scheduler(time.time, time.sleep)
    ...
    def _update_throughput(
        ...
        if self._simulate and self._estimate_throughputs:
            ...
            if job_id.is_pair():
                old_throughput = copy.deepcopy(self._throughputs[job_id][worker_type])
            else:
    ...
    def add_job(self, job, timestamp=None):
        ...
        with self._scheduler_lock:
            ...
            if self._SLOs is not None:
                ...
                self._SLOs[job_id] = (
                    job.SLO * job.duration + self.get_current_timestamp(in_seconds=True)
                )
            for worker_type in self._worker_types:
            ...
            if not self._simulate and job_id in completed_jobs:
                self._logger.debug(
                    ...
                    "{num_gpus} {worker_type} GPUs".format(
                        job_id=job_id, num_gpus=len(worker_ids), worker_type=worker_type
                    )
                )
                continue
            allocation_str = ""
    ...
    def _mid_round(self, pool):
        ...
        round_end_time = self._current_round_start_time + self._time_per_iteration
        ...
        for job_id in self._current_worker_assignments:
            ...
            if (
                ...
                if current_worker_ids == next_worker_ids:
                    ...
                    self._logger.info("Extending lease for job {0}".format(job_id))
                    self._num_lease_extensions += 1
    ...
    def _schedule_completion_events(self, round_end_time, pool):
        ...
        for job_id in self._current_worker_assignments:
            is_active = any([x in self._jobs for x in job_id.singletons()])
            if not is_active or job_id in self._completed_jobs_in_current_round:
                continue
    ...
    def _end_round(self):
        ...
        if not self._is_final_round():
            ...
            if self._next_worker_assignments is None:
                raise RuntimeError("Next worker assignments have not been computed!")
            ...
            if remaining_time_in_round > 0:
                self._logger.debug(
                    ...
                    "round {1}...".format(remaining_time_in_round, current_round + 1)
                )
        ...
        with self._scheduler_lock:
            ...
            for job_id in job_ids:
                ...
                if self._job_priority_weights[job_id] == 1.0:
                    ...
                    print("Job %s (high priority): %.3f" % (job_id, completion_time))
            ...
            if verbose:
                ...
                if len(low_priority_job_completion_times) > 0:
                    average_low_pri_jct = np.mean(low_priority_job_completion_times)
                    print(
                ...
                if len(high_priority_job_completion_times) > 0:
                    average_high_pri_jct = np.mean(high_priority_job_completion_times)
                    print(
    ...
    def save_job_timelines(self, timeline_dir):
        ...
        for job_id in sorted(self._job_timelines.keys()):
            ...
            for i in range(len(self._job_timelines[job_id])):
                timeline_file = os.path.join(job_dir, "worker={0}.log".format(i))
                ...
                if (
                    ...
                    if not self._simulate:
                        if (
                            ...
                            job_time_so_far += elapsed_job_time[job_id][worker_type]
    ...
    def _add_available_worker_id(self, worker_id):
        ...
        if not self._simulate:
            self._logger.debug("Adding worker {0} back to queue...".format(worker_id))
        self._available_worker_ids.put(worker_id)
        if not self._simulate:
            self._logger.debug("Added worker {0} back to queue".format(worker_id))
    ...
    def _init_job_callback(self, job_id):
        ...
        with self._scheduler_cv:
            ...
            self._per_job_latest_timestamps[job_id] = self.get_current_timestamp()
    ...
    def _update_lease_callback(
        ...
        with self._scheduler_lock:
            ...
            remaining_time_in_current_round = current_round_end_time - current_time
            remaining_time_in_current_round = max(0, remaining_time_in_current_round)
            ...
            if update_id == 0:
                with self._scheduler_lock:
                    ...
                    self._max_steps[job_id] = min(
                        remaining_steps,
                        steps + int(remaining_time_in_current_round * throughput),
                    )
    ...
    def _kill_job(self, job_id):
        with self._scheduler_cv:
            ...
            self._logger.debug("Waiting for job {0} to be killed...".format(job_id))
            self._scheduler_cv.wait(timeout=30)
            self._logger.debug("Checking if job {0} was killed...".format(job_id))
            ...
            if successful_kill:
                ...
                worker_ids_to_complete = all_worker_ids.difference(completed_worker_ids)
                self._logger.debug(
                    "Need to send done callbacks for the following "
                    "workers for job {0}: {1}".format(job_id, worker_ids_to_complete)
                )
    ...
    def _done_callback_extended_lease(self, job_id):
        ...
        with self._scheduler_cv:
            ...
            num_updates = []
            for single_job_id in job_id.singletons():
                num_updates.append(len(self._lease_update_requests[single_job_id]))
        ...
        with self._scheduler_lock:
            ...
            if not self._simulate:
                while (
                    ...
                    if job_id not in self._current_worker_assignments and (
                        ...
                        return
                    self._logger.debug("Waiting to complete job {0}...".format(job_id))
                    self._scheduler_cv.wait()
            ...
            if not any(is_active.values()):
                self._logger.info(
                    ...
                    "already completed!".format(job_id=job_id, worker_id=worker_id)
                )
                return
            ...
            if not micro_task_succeeded:
                # Micro-task failed.
                self._logger.info(
                    "[Micro-task failed]\tJob ID: {job_id}".format(job_id=job_id)
                )
                            ...
                            if not self._simulate:
                                self._logger.debug(
                                    ...
                                    "remaining steps".format(
                                        job_id=single_job_id, steps=remaining_steps
                                    )
                                )
                        else:
