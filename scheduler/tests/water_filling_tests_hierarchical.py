...
def test_water_filling_multilevel():
    ...
    for i in range(num_entities):
        ...
        priority_reweighting_policies[entity_id] = ["fifo", "fairness"][
            random.randint(0, 1)
        ]
        entity_to_job_mapping[entity_id] = []
        entity_weights[entity_id] = random.randint(1, 5)
        print(
            "Entity %d: Priority=%d, Policy=%s"
            % (i, entity_weights[entity_id], priority_reweighting_policies[entity_id])
        )
    ...
    for i in range(num_jobs):
        ...
        print(
            "Job %d: Throughputs=%s, Priority=%d, Scale factor=%d, Entity=%s"
            % (
                i,
                unflattened_throughputs[i],
                unflattened_priority_weights[i],
                scale_factors[i],
                entity_id.replace("entity", ""),
            )
        )
