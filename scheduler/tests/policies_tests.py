...
class TestPolicies(unittest.TestCase):
    ...
    def test_allox(self):
        ...
        cluster_spec = {"v100": 2, "p100": 1, "k80": 3}
        allocation1 = policy.get_allocation(
            unflattened_throughputs,
            scale_factors,
            times_since_start,
            num_steps_remaining,
        ...
        allocation2 = policy.get_allocation(
            unflattened_throughputs,
            scale_factors,
            times_since_start,
            num_steps_remaining,
    ...
    def test_isolated(self):
        ...
        max_min_fairness_policy = max_min_fairness.MaxMinFairnessPolicy(solver="ECOS")
        unflattened_throughputs = {
            0: {"v100": 2.0, "p100": 1.0, "k80": 0.5},
            1: {"v100": 3.0, "p100": 2.0, "k80": 1.0},
        }
        unflattened_priority_weights = {0: 1, 1: 1}
        all_scale_factors = [{0: 1, 1: 1}, {0: 2, 1: 1}, {0: 2, 1: 2}, {0: 4, 1: 1}]
        all_cluster_specs = [
            {"v100": 4, "p100": 4, "k80": 4},
            {"v100": 4, "p100": 8, "k80": 6},
        ]
        for scale_factors, cluster_spec in itertools.product(
            ...
            for job_id in unflattened_throughputs:
                ...
                for worker_type in cluster_spec:
                    isolated_objective += (
                        isolated_allocation[job_id][worker_type] * scale_factors[job_id]
                    )
            ...
            assert np.isclose(
                min(isolated_objectives), min(max_min_fairness_objectives)
            )
    ...
    def test_gandiva(self):
        ...
        scale_factors = {JobIdPair(0, None): 1, JobIdPair(1, None): 1}
        cluster_spec = {"v100": 1, "p100": 1, "k80": 1}
        policy.get_allocation(unflattened_throughputs, scale_factors, cluster_spec)
    ...
    def test_max_min_fairness_with_perf(self):
        ...
        unflattened_priority_weights = {0: 1, 1: 1}
        cluster_spec = {"v100": 1, "p100": 2, "k80": 3}
    ...
    def test_max_min_fairness_with_packing(self):
        policy = max_min_fairness.MaxMinFairnessPolicyWithPacking(solver="ECOS")
        unflattened_throughputs = {
            JobIdPair(0, None): {"v100": 2.0, "p100": 1.0, "k80": 0.5},
            JobIdPair(1, None): {"v100": 3.0, "p100": 2.0, "k80": 1.0},
            JobIdPair(0, 1): {
        ...
        scale_factors = {JobIdPair(0, None): 1, JobIdPair(1, None): 1}
        unflattened_priority_weights = {JobIdPair(0, None): 1, JobIdPair(1, None): 1}
        cluster_spec = {"v100": 1, "p100": 1, "k80": 1}
        policy.get_allocation(
            unflattened_throughputs,
            scale_factors,
    ...
    def test_max_min_fairness_water_filling_with_packing(self):
        policy = (
            max_min_fairness_water_filling.MaxMinFairnessWaterFillingPolicyWithPacking()
        )
        unflattened_throughputs = {
            JobIdPair(0, None): {"v100": 2.0, "p100": 1.0, "k80": 0.5},
            JobIdPair(1, None): {"v100": 3.0, "p100": 2.0, "k80": 1.0},
    ...
    def test_finish_time_fairness_with_packing(self):
        policy = finish_time_fairness.FinishTimeFairnessPolicyWithPacking(solver="ECOS")
        unflattened_throughputs = {
            JobIdPair(0, None): {"v100": 2.0, "p100": 1.0, "k80": 0.5},
            JobIdPair(1, None): {"v100": 3.0, "p100": 2.0, "k80": 1.0},
            JobIdPair(0, 1): {
    ...
    def test_throughput_sum(self):
        ...
        cluster_spec = {"v100": 1, "p100": 1, "k80": 1}
        policy.get_allocation(
            unflattened_throughputs, scale_factors, cluster_spec, instance_costs=None
        )
