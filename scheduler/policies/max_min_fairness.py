...
class MaxMinFairnessPolicy(Policy):
    ...
    def __init__(self, solver):
        ...
        self._max_min_fairness_perf_policy = MaxMinFairnessPolicyWithPerf(solver)

    def get_allocation(
        self, unflattened_throughputs, scale_factors, priority_weights, cluster_spec
    ):
        ...
        return self._max_min_fairness_perf_policy.get_allocation(
            new_unflattened_throughputs, scale_factors, priority_weights, cluster_spec
        )
...
class MaxMinFairnessPolicyWithPacking(PolicyWithPacking):
    ...
    def get_allocation_using_job_type_throughputs(
        ...
        flattened_throughputs = np.zeros(shape=(a, (1 + a) * m), dtype=np.float32)
        ...
        for i in range(m):
            relevant_vars = x[:, i * num_vars_per_job : (i + 1) * num_vars_per_job]
            ...
            per_worker_type_allocations.append(
                cp.sum(
                    cp.multiply(
                        relevant_vars, cp.multiply(relevant_scale_factors, masks)
                    )
                )
            )
        constraints.append(cp.hstack(per_worker_type_allocations) <= num_workers)
