...
class GandivaPolicy(PolicyWithPacking):
    ...
    def get_allocation(self, unflattened_throughputs, scale_factors, cluster_spec):
        all_throughputs, index = super().flatten(unflattened_throughputs, cluster_spec)
        ...
        if num_workers_requested <= num_workers_available:
            ...
            x = self._get_allocation(single_job_ids, index, scale_factors, cluster_spec)
        else:
            # Deploy packing.
            ...
            while len(to_be_assigned_combinations) > 1 and i < n:
                i += 1
                [job1_id, job2_id] = self._rng.sample(to_be_assigned_combinations, 2)
                ...
                self._assigned_combinations[job1_id] = (job_combination, job2_id)
                self._assigned_combinations[job2_id] = (job_combination, job1_id)
            ...
            x = self._get_allocation(
                job_combinations_to_schedule, index, scale_factors, cluster_spec
            )
