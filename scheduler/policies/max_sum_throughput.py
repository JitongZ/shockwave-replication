...
class ThroughputNormalizedByCostSumWithPerf(Policy):
    ...
    def get_allocation(
        self, unflattened_throughputs, scale_factors, cluster_spec, instance_costs
    ):
        return self._policy.get_allocation(
...
class ThroughputNormalizedByCostSumWithPerfSLOs(Policy):
    ...
    def get_allocation(
        ...
        objective = cp.Maximize(
            cp.sum(cp.sum(cp.multiply(throughputs / instance_costs_array, x), axis=1))
        )
...
class ThroughputNormalizedByCostSumWithPackingSLOs(PolicyWithPacking):
    ...
    def get_allocation(
        ...
        if instance_costs is not None:
            for i in range(m):
                for j in range(n):
                    instance_costs_array[i, j] = instance_costs[worker_types[j]]
        ...
        for i in range(len(single_job_ids)):
            ...
            objective_terms.append(
                cp.sum(
                    cp.multiply(
                        all_throughputs[i][indexes] / instance_costs_array[indexes],
                        x[indexes],
                    )
                )
            )
        ...
        for job_id in SLOs:
            ...
            assert job_id in num_steps_remaining
            indexes = relevant_combinations[single_job_ids[i]]
            throughput = cp.sum(cp.multiply(all_throughputs[i][indexes], x[indexes]))
