...
class WaterFillingAlgorithm:
    ...
    def _compute_priority_weights(
        ...
        for entity_id in entity_to_job_mapping:
            priority_reweighting_policy = self._priority_reweighting_policies[entity_id]
    ...
    def _get_allocation(
        ...
        normalized_effective_throughputs = cp.multiply(
            effective_throughputs, 1.0 / proportional_throughputs.reshape(len(job_ids))
        )
    ...
    def _get_bottleneck_jobs(
        ...
        if self._milp is None:
            ...
            self._milp_variables_and_parameters["mask_parameter"] = cp.Parameter(
                len(job_ids)
            )
...
class MaxMinFairnessWaterFillingPolicyWithPacking(
    ...
    def _get_constraints(self, x, scale_factors_array):
        return self.get_base_constraints(
            x, self._single_job_ids, scale_factors_array, self._relevant_combinations
        )

    def _get_M(
        self, all_throughputs, index, proportional_throughputs, scale_factors_array
    ):
        ...
        for i, single_job_id in enumerate(single_job_ids):
            ...
            max_throughputs.append(
                np.max(
                    np.multiply(
                        all_throughputs[i][indexes], scale_factors_array[indexes]
                    )
                )
            )
    ...
    def get_allocation(
        ...
        proportional_throughputs = self._proportional_policy.get_throughputs(
            throughputs_no_packed_jobs, (single_job_ids, worker_types), cluster_spec
        )

        self._M = self._get_M(
            all_throughputs, index, proportional_throughputs, scale_factors_array
        )
        self._get_effective_throughputs = (
            lambda x: self._get_effective_throughputs_helper(x, all_throughputs, index)
        )
