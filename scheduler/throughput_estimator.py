...
class ThroughputEstimator:
    ...
    def _get_normalized_throughputs(self):
        ...
        for i, job_type in enumerate(self._job_types):
            for j, worker_type in enumerate(self._worker_types):
                per_worker_oracle_throughputs = self._oracle_throughputs[worker_type][
                    job_type
                ]
    ...
    def _get_reference_throughputs(self, num_reference_job_types):
        ...
        reference_job_types_idx = sorted(
            self._rng.sample(list(range(len(self._job_types))), num_reference_job_types)
        )
        ...
        if len(self._reference_job_types) == len(self._job_types):
            ...
            assert (self._normalized_throughputs == self._reference_throughputs).all()
    ...
    def match_job_to_reference_job(self, true_job_type):
        ...
        for i, worker_type in enumerate(sorted(profiled_jobs.keys())):
            for j, reference_job_type in enumerate(self._reference_job_types):
                if reference_job_type in profiled_jobs[worker_type]:
                    offset = i * len(self._reference_job_types) + j
                    throughputs_matrix[-1][offset] = profiled_jobs[worker_type][
                        reference_job_type
                    ]
                    mask[-1][offset] = 1
        ...
        if np.min(mask) == 0:
            ...
            with warnings.catch_warnings():
                ...
                try:
                    ...
                    throughputs_matrix = np.where(
                        mask, throughputs_matrix, np.clip(estimated_throughputs, 0, 1)
                    )[0]
                except np.linalg.LinAlgError as e:
                    if self._verbose:
                        print(
                            "WARNING: could not estimate throughputs!", file=sys.stderr
                        )
            ...
            if self._verbose:
                print(
                    "WARNING: Did not run matrix completion as " "mask is complete",
                    file=sys.stderr,
                )
