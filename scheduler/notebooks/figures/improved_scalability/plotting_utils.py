...
def plot_effective_throughput_ratios(
    all_effective_throughputs, cdf=False, output_filename=None
):
    plt.figure(figsize=(6.5, 3))
    ax = plt.subplot2grid((1, 1), (0, 0), colspan=1)
    ...
    for effective_throughputs, label in zip(
        ...
        if cdf:
            ax.plot(
                [
                    float((i + 1) * 100.0) / len(effective_throughputs)
                    for i in range(len(effective_throughputs))
                ],
                sorted(effective_throughput_ratios),
                label=label,
            )
        else:
            ax.plot(
                range(len(effective_throughputs)),
                effective_throughput_ratios,
                label=label,
            )
...
def plot_runtime_vs_effective_throughput_ratios(
    runtimes, all_effective_throughputs, labels, draw_arrow=False, output_filename=None
):
    plt.figure(figsize=(6.5, 3))
    ax = plt.subplot2grid((1, 1), (0, 0), colspan=1)
    job_ids = list(all_effective_throughputs[0].keys())
