...
def create_problem_instance(
    num_jobs, cluster_spec, policy_name, seed, introduce_skew=False
):
    ...
    for i in range(num_jobs):
        ...
        job = utils.generate_job(throughputs=oracle_throughputs, rng=rng, job_id=job_id)
        jobs[job_id[0]] = job
        ...
        for i in range(num_jobs):
            ...
            for j in range(num_jobs):
                if i < j and jobs[i].scale_factor == jobs[j].scale_factor:
                    other_job_type_key = (jobs[j].job_type, jobs[j].scale_factor)
                    throughputs[JobIdPair(i, j)] = {}
                    for worker_type in cluster_spec:
                        throughputs[JobIdPair(i, j)][worker_type] = oracle_throughputs[
                            worker_type
...
def harness(
    ...
    for i, job_id in enumerate(job_ids):
        if random_cluster_assignment:
            job_to_sub_cluster_assignment[job_id[0]] = random.randint(
                0, num_sub_clusters - 1
            )
        else:
    ...
    sub_cluster_cluster_spec = {
        worker_type: cluster_spec[worker_type] // num_sub_clusters
        for worker_type in cluster_spec
    }
...
def sweep(
    policy_names_and_num_sub_clusters, all_num_jobs, num_trials, introduce_skew=False
):
    all_runtimes = {}
    ...
    for num_jobs in all_num_jobs:
        ...
        for i in range(num_trials):
            throughputs, scale_factors, priority_weights = create_problem_instance(
                num_jobs,
                cluster_spec,
                policy_names_and_num_sub_clusters[0][0],
                seed=i,
...
def get_runtimes_and_effective_throughputs(
    policy_name, all_num_sub_clusters, num_jobs, introduce_skew=False
):
    policy_names_and_num_sub_clusters = [
        (policy_name, num_sub_clusters) for num_sub_clusters in all_num_sub_clusters
    ]
    if not introduce_skew:
        policy_names_and_num_sub_clusters.append(("gandiva", 1))
...
if __name__ == "__main__":
    ...
    runtimes, all_effective_throughputs = get_runtimes_and_effective_throughputs(
        "max_min_fairness_packed", all_num_sub_clusters, 1024
    )
    print(runtimes)
