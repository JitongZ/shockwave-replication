...
def get_logfile_paths(directory_name, static_trace=False):
    ...
    for logfile_path in get_logfile_paths_helper(directory_name):
        ...
        logfile_paths.append(
            (v100s, p100s, k80s, policy, seed, lambda_or_num_total_jobs, logfile_path)
        )
    return logfile_paths
...
def prune(logfile_paths, v100s, p100s, k80s, policy, seed=None):
    if seed is None:
        return sorted(
            [
                (x[5], x[6], x[4])
                for x in logfile_paths
                if x[0] == v100s and x[1] == p100s and x[2] == k80s and x[3] == policy
            ]
        )
    else:
        return sorted(
...
def average_jct_low_priority_fn(logfile_path, min_job_id=None, max_job_id=None):
    job_completion_times = []
    with open(logfile_path, "rb") as f:
        ...
        for line in lines[-5:]:
            m = re.match(
                r"Average job completion time \(low priority\): (\d+\.\d+) seconds",
                line,
            )
            if m is not None:
...
def average_jct_high_priority_fn(logfile_path, min_job_id=None, max_job_id=None):
    job_completion_times = []
    with open(logfile_path, "rb") as f:
        f.seek(-8192, os.SEEK_END)
        text = f.read().decode("utf-8")
        ...
        for line in lines[-5:]:
            m = re.match(
                r"Average job completion time \(high priority\): (\d+\.\d+) seconds",
                line,
            )
            if m is not None:
                return float(m.group(1)) / 3600
...
def makespan_fn(logfile_path):
    ...
    with open(logfile_path, "r") as f:
        ...
        for line in lines[-10000:]:
            ...
            if m is not None:
                makespan = float(m.group(1)) / 3600.0
                return makespan
    return None
            ...
            if m is not None:
                ...
                if min_job_id is None or min_job_id <= job_id:
                    if max_job_id is None or job_id <= max_job_id:
                        ...
                        job_completion_times.append((job_completion_time, job_duration))
