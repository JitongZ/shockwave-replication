...
def plot_metric_vs_inverse_lambda_different_mechanisms(
    all_logfile_paths,
    labels,
    label_modifiers,
    v100s,
    ...
    print(policies)
    for policy in policies:
        for logfile_paths, label_modifier in zip(all_logfile_paths, label_modifiers):
            relevant_logfile_paths = list(
                reversed(prune(logfile_paths, v100s, p100s, k80s, policy))
            )
...
def plot_metric_vs_inverse_lambda_different_metric_fns(
    logfile_paths,
    labels,
    v100s,
    p100s,
    ...
    for policy in policies:
        ...
        for metric_fn_label, metric_fn in zip(metric_fn_labels, metric_fns):
            ...
            seeds = [x[2] for x in relevant_logfile_paths]
            policies = [
                labels[policy] + " (%s)" % metric_fn_label for i in range(len(metrics))
            ]
...
def plot_jct_cdf(
    ...
    for l in lambdas:
        ...
        if finish_time_fairness:
            relevant_logfile_paths = list(
                reversed(prune(logfile_paths, v100s, p100s, k80s, "isolated", seed=0))
            )
            relevant_logfile_paths = [x for x in relevant_logfile_paths if x[0] == l]
            if len(relevant_logfile_paths) != 1:
                continue
            isolated_jcts = get_jcts(
                relevant_logfile_paths[0][1],
                seed=0,
        ...
        for i, policy in enumerate(policies):
            relevant_logfile_paths = list(
                reversed(prune(logfile_paths, v100s, p100s, k80s, policy, seed=0))
            )
            relevant_logfile_paths = [x for x in relevant_logfile_paths if x[0] == l]
            if len(relevant_logfile_paths) != 1:
                continue
            ...
            for j, (ax, jcts_segment) in enumerate(zip(axes, jcts)):
                jcts_segment.sort()
                percentiles = [
                    (k + 1) / len(jcts_segment) for k in range(len(jcts_segment))
                ]
        ...
        if output_directory is not None:
            output_filename = os.path.join(
                output_directory, "input_job_rate=%d.pdf" % (input_job_rate * 10)
            )
            with PdfPages(output_filename) as pdf:
                pdf.savefig(bbox_inches="tight")
