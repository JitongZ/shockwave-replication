...
def plot_data(data, metrics_to_plot, save_dir, metrics_names):
    ...
    for j, metric in enumerate(metrics_to_plot):
        ...
        for idx, policy in enumerate(policies):
            values = np.mean(data[num_gpus_list[0]][policy][metric]) if metric in data[num_gpus_list[0]][policy] else 0
            ax.barh(bar_positions[idx], values, color=colors[idx], height=0.01, align='center', edgecolor='black')
    ...
    labels = policies
    legend = fig.legend(handles, labels, loc='upper center', ncol=len(policies), frameon=True, fontsize='large', bbox_to_anchor=(0.5, 0.98))
