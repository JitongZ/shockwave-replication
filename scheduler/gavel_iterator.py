...
class GavelIterator:
    def __init__(
        ...
        if not isinstance(data_loader, Iterable):
            raise ValueError("Data is of uniterable " "type %s" % (type(data_loader)))
        else:
    ...
    def _write_info(self):
        self._logger.info(
            "{0}".format(self._steps), extra={"event": "PROGRESS", "status": "STEPS"}
        )
        self._logger.info(
    ...
    def _update_lease(self, init=False):
        if init:
            ...
            (updated_max_steps, updated_max_duration) = self._rpc_client.update_lease(
                self._steps,
                self._duration,
                self._lease.max_steps,
                self._lease.max_duration,
        ...
        if updated_max_duration <= self._lease.max_duration:
            ...
            additional_lease_time = updated_max_duration - self._lease.max_duration
            time_left_on_current_lease = self._lease.max_duration - self._duration
            ...
            self._logger.debug(
                "Progress: steps={0}, duration={1}".format(self._steps, self._duration),
