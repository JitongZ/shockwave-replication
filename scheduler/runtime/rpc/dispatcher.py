...
class Dispatcher:
    def __init__(
        ...
        ch.setFormatter(logging.Formatter(LOG_FORMAT, datefmt=DATE_FORMAT, style="{"))
    ...
    def _configure_numa(self):
        self._numa_available = numa.available() and which("numactl") is not None
        ...
        for i in range(num_numa_nodes):
            ...
            for j in range(1, len(cpus)):
                if (
                    ...
                    contiguous_blocks.append((contiguous_block, len(contiguous_block)))
    ...
    def _construct_command(self, job, gpu_id, worker_id):
        checkpoint_dir = os.path.join(self._checkpoint_dir, "job_id=%d" % (job.job_id))
        with self._lock:
    ...
    def _get_steps_and_execution_time(self, job_id, worker_id, round_id):
        ...
        with open(log_file, "r") as f:
            for line in f:
                match = re.match("\[(.*)\] \[(.*)\] \[(.*)\]\ ?(.*)", line)
                if match is None:
                    self._logger.error("Malformed Gavel log file: {0}".format(line))
                    continue
    ...
    def _kill_job(self, pid):
        ...
        try:
            ...
            self._logger.error("Could not kill process {0}: {1}".format(pid, e))
    ...
    def _kill_jobs(self, job_id=None):
        with self._lock:
            ...
            if job_id is not None:
                if job_id not in self._commands or len(self._commands[job_id]) == 0:
                    self._logger.warning("No commands found for job {0}".format(job_id))
                    return
                ...
                for command in self._commands[job_id][round_id]:
                    ...
                    if pid is not None:
                        pids.append(pid)
                self._logger.debug(
                    "PIDs for job {0} in round {1}: {2}".format(job_id, round_id, pids)
                )
                for pid in pids:
                    self._kill_job(pid)
            else:
    ...
    def _get_job_logger_and_fh(self, job_id, worker_id, round_id):
        ...
        fh.setFormatter(
            logging.Formatter(ITERATOR_LOG_FORMAT, datefmt=DATE_FORMAT, style="{")
        )
    ...
    def launch_job(self, job, prefix, command, worker_id, round_id, gpu_id):
        ...
        # Log job dispatch.
        job_logger, fh = self._get_job_logger_and_fh(job.job_id, worker_id, round_id)
        ...
        try:
            ...
            error_message = (
                "Job {job_id} (worker {worker_id}, " "round {round_id}) failed!"
        ...
        job_logger.info("", extra={"event": "DISPATCHER", "status": "COMPLETE"})
        ...
        try:
            completed_steps, execution_time = self._get_steps_and_execution_time(
    ...
    def _dispatch_jobs_helper(self, jobs, worker_id, round_id):
        ...
        self._logger.debug(
            ...
            "round {round_id})".format(
                gpu_id=gpu_id, job_id=job_ids, worker_id=worker_id, round_id=round_id
            )
        )
        with self._lock:
        ...
        for job in jobs:
            try:
                ...
                self._logger.error(
                    "Failed to construct command " "for job {0}!".format(job.job_id)
                )
                traceback.print_exc()
                success = False
                break
