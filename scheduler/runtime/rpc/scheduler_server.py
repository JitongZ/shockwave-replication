    ...
    def RegisterWorker(self, request, context):
        ...
        try:
            ...
            return w2s_pb2.RegisterWorkerResponse(
                success=True, worker_ids=worker_ids, round_duration=round_duration
            )
        except Exception as e:
            self._logger.error("Could not register worker: {0}".format(e))
            return w2s_pb2.RegisterWorkerResponse(successful=False, error_message=e)
...
class SchedulerIteratorRpcServer(i2s_pb2_grpc.IteratorToSchedulerServicer):
    ...
    def InitJob(self, request, context):
        ...
        return i2s_pb2.UpdateLeaseResponse(
            max_steps=max_steps, max_duration=max_duration, extra_time=extra_time
        )
    ...
    def UpdateLease(self, request, context):
        ...
        try:
            (max_steps, max_duration) = update_lease_callback(
                job_id=job_id,
                worker_id=request.worker_id,
                steps=request.steps,
                duration=request.duration,
            ...
            self._logger.error(
                "Could not update lease for job {0}: {1}".format(job_id, str(e))
            )
            max_steps = request.max_steps
            max_duration = request.max_duration
