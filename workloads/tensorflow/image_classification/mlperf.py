...
_ModelInfo = namedtuple("_ModelInfo", ["print_fn", "tag_set", "mlperf_model_name"])
...
class MlPerfLogger(object):
    ...
    def __init__(self, model):
        ...
        self.model = model
        model_to_info = {
            "resnet50_v1.5": _ModelInfo(
                mlperf_log.resnet_print, mlperf_log.RESNET_TAG_SET, tags.RESNET
            ),
            "ssd300": _ModelInfo(
                mlperf_log.ssd_print, mlperf_log.SSD_TAG_SET, tags.SSD
            ),
        }
        ...
        try:
            ...
            raise ValueError(
                ...
                "--model is one of the following: " + ", ".join(model_to_info.keys())
            )
    ...
    def log_deferred_tensor_value(
        self, key, tensor_value, global_step, stack_offset=2, every_n=1
    ):
        ...
        maybe_print = tf.cond(
            tf.equal(global_step % every_n, 0), create_print_op, tf.no_op
        )
    ...
    def log_conv2d(
        ...
        if self.model == "resnet50_v1.5":
            ...
            if isinstance(initializer, tf.truncated_normal_initializer) or (
                isinstance(initializer, tf.variance_scaling_initializer)
                and initializer.distribution == "truncated_normal"
            ):
                initializer = tags.TRUNCATED_NORMAL
            elif (
                isinstance(initializer, tf.glorot_uniform_initializer)
    ...
    def log_batch_norm(
        self, input_tensor, output_tensor, momentum, epsilon, center, scale, training
    ):
        if self.model == "resnet50_v1.5":
    ...
    def log_train_epochs(self, num_epochs):
        ...
        if num_epochs_int != num_epochs:
            value = str(
                num_epochs_int
            ) + ", but this epoch only has {}% of the examples of a normal epoch".format(
                100 * (num_epochs - num_epochs_int)
            )
    ...
    def log_input_resize_aspect_preserving(self, height, width, scale_factor):
        assert height == width, (
            ...
            "images. Cannot process image with height=%d and width=%d" % (height, width)
        )
        self.log(
            key=tags.INPUT_RESIZE_ASPECT_PRESERVING,
    ...
    def log_eval_accuracy(
        self, accuracy, global_step, batch_size, examples_per_epoch, stack_offset=2
    ):
        """Logs eval accuracy."""
        ...
        self.log(
            key=tags.EVAL_ACCURACY, value=eval_accuracy, stack_offset=stack_offset + 1
        )
        self.log(
            key=tags.EVAL_ITERATION_ACCURACY,
