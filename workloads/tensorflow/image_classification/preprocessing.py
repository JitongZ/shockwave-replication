...
def normalized_image(images):
    ...
    images = tf.multiply(images, 1.0 / 127.5)
    # Rescale to [-1, 1]
...
def eval_image(
    image, height, width, batch_position, resize_method, summary_verbosity=0
):
    ...
    with tf.name_scope("eval_image"):
        ...
        mlperf.logger.log_input_resize_aspect_preserving(height, width, scale_factor)
        ...
        distorted_image = tf.slice(
            distorted_image, [crop_top, crop_left, 0], [height, width, 3]
        )
...
def train_image(
    ...
    with tf.name_scope(scope or "distort_image"):
        ...
        max_attempts = 100
        mlperf.logger.log(
            key=mlperf.tags.INPUT_DISTORTED_CROP_MIN_OBJ_COV, value=min_object_covered
        )
        mlperf.logger.log(
            key=mlperf.tags.INPUT_DISTORTED_CROP_RATIO_RANGE, value=aspect_ratio_range
        )
        mlperf.logger.log(
            key=mlperf.tags.INPUT_DISTORTED_CROP_AREA_RANGE, value=area_range
        )
        mlperf.logger.log(
            key=mlperf.tags.INPUT_DISTORTED_CROP_MAX_ATTEMPTS, value=max_attempts
        )
        ...
        image_resize_method = get_image_resize_method(resize_method, batch_position)
        distorted_image = tf.image.resize_images(
            distorted_image, [height, width], image_resize_method, align_corners=False
        )
        ...
        if summary_verbosity >= 3:
            tf.summary.image(
                "cropped_resized_maybe_flipped_image",
    ...
    def build_multi_device_iterator(
        ...
        with tf.name_scope("batch_processing"):
            ...
            tf.add_to_collection(
                tf.GraphKeys.TABLE_INITIALIZERS, multi_device_iterator.initializer
            )
...
class RecordInputImagePreprocessor(BaseImagePreprocessor):
    ...
    def preprocess(self, image_buffer, bbox, batch_position):
        ...
        if self.match_mlperf:
            mlperf.logger.log(
                key=mlperf.tags.INPUT_MEAN_SUBTRACTION, value=_CHANNEL_MEANS
            )
        ...
        with tf.name_scope("batch_processing"):
            ...
            for i in xrange(self.batch_size):
                ...
                raw_image = tf.reshape(
                    raw_images[i], [dataset.depth, dataset.height, dataset.width]
                )
...
class COCOPreprocessor(BaseImagePreprocessor):
    ...
    def minibatch(self, dataset, subset, params, shift_ratio=-1):
        ...
        with tf.name_scope("batch_processing"):
            ...
            input_lists = [
                [None for _ in range(self.num_splits)] for _ in range(input_len)
            ]
            for d in xrange(self.num_splits):
    ...
    def preprocess(self, data):
        ...
        if self.train:
            ...
            image, boxes = preprocessor.random_horizontal_flip(image=image, boxes=boxes)
            ...
            def trim_and_pad(inp_tensor):
                ...
                return tf.reshape(
                    inp_tensor,
                    [ssd_constants.MAX_NUM_EVAL_BOXES, inp_tensor.get_shape()[1]],
                )
...
class TestImagePreprocessor(BaseImagePreprocessor):
    ...
    def minibatch(self, dataset, subset, params, shift_ratio=0):
        ...
        fake_images = cnn_util.roll_numpy_batches(
            self.fake_images, self.batch_size, shift_ratio
        )
        fake_labels = cnn_util.roll_numpy_batches(
            self.fake_labels, self.batch_size, shift_ratio
        )
...
class LibrispeechPreprocessor(InputPreprocessor):
    ...
    def __init__(self, batch_size, output_shapes, num_splits, dtype, train, **kwargs):
        del kwargs
    ...
    def parse_and_preprocess(self, value, batch_position):
        ...
        return [
            ...
            # Label
            tf.cast(
                tf.reshape(tf.sparse_tensor_to_dense(context_parsed["labels"]), [-1]),
                dtype=tf.int32,
            ),
            # Input length
            tf.cast(tf.reshape(context_parsed["input_length"], [1]), dtype=tf.int32),
            # Label length
            tf.cast(tf.reshape(context_parsed["label_length"], [1]), dtype=tf.int32),
        ]
