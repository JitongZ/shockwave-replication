    ...
    with tf.variable_scope(name):
        if depth == in_size:
            if stride == 1:
                ...
                mlperf.logger.log_projection(
                    input_tensor=input_layer, output_tensor=shortcut
                )
        else:
            shortcut = cnn.conv(
                depth,
...
def bottleneck_block(cnn, depth, depth_bottleneck, stride, version):
    ...
    """
    mlperf.logger.log(
        key=mlperf.tags.MODEL_HP_BLOCK_TYPE, value=mlperf.tags.BOTTLENECK_BLOCK
    )
...
def residual_block(cnn, depth, stride, version, projection_shortcut=False):
    ...
    if projection_shortcut:
        ...
        # Plan A of shortcut.
        shortcut = cnn.apool(
            1, 1, stride, stride, input_layer=input_layer, num_channels_in=in_size
        )
...
class ResnetModel(model_lib.CNNModel):
    ...
    def __init__(self, model, layer_counts, params=None):
        ...
        super(ResnetModel, self).__init__(
            model, 224, batch_size, base_lr, layer_counts, params=params
        )
        if "v2" in model:
            self.version = "v2"
        elif "v1.5" in model:
    ...
    def add_inference(self, cnn):
        ...
        mlperf.logger.log(
            key=mlperf.tags.MODEL_HP_INITIAL_SHAPE,
    ...
    def get_learning_rate(self, global_step, batch_size):
        ...
        num_batches_per_epoch = datasets.IMAGENET_NUM_TRAIN_IMAGES / batch_size
...
class ResnetCifar10Model(model_lib.CNNModel):
    ...
    def get_learning_rate(self, global_step, batch_size):
        ...
        boundaries = num_batches_per_epoch * np.array([82, 123, 300], dtype=np.int64)
