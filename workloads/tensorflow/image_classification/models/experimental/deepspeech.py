...
class DeepSpeech2Model(model_lib.Model):
    ...
    def _conv_bn_layer(
        self, inputs, padding, filters, kernel_size, strides, layer_id, training
    ):
    ...
    def get_synthetic_inputs(self, input_name, nclass):
        ...
        input_lengths = tf.convert_to_tensor([self.max_time_steps] * self.batch_size)
        label_lengths = tf.convert_to_tensor([self.max_label_length] * self.batch_size)
    ...
    def build_network(self, inputs, phase_train=True, nclass=29):
        ...
        inputs = tf.reshape(
            inputs, [self.batch_size, -1, feat_size * DeepSpeech2Model.CONV_FILTERS]
        )
    ...
    def loss_function(self, inputs, build_network_result):
        ...
        ctc_input_length = tf.to_float(tf.multiply(actual_time_steps, ctc_time_steps))
    ...
    def accuracy_function(self, inputs, logits):
        ...
        return {
            (constants.UNREDUCED_ACCURACY_OP_PREFIX + self.PROBABILITY_TENSOR): probs,
            (constants.UNREDUCED_ACCURACY_OP_PREFIX + self.LABEL_TENSOR): inputs[1],
        }
    ...
    def postprocess(self, results):
        ...
        for i in range(self.batch_size):
            ...
            # Compute CER.
            total_cer += greedy_decoder.cer(predicted_str, expected_str) / len(
                expected_str
            )
            # Compute WER.
            total_wer += greedy_decoder.wer(predicted_str, expected_str) / len(
                expected_str.split()
            )
        ...
        log_fn(
            "total CER: {:f}; total WER: {:f}; total example: {:d}.".format(
