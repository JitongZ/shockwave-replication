...
class SSD300Model(model_lib.CNNModel):
    ...
    def __init__(
        self,
        label_num=ssd_constants.NUM_CLASSES,
        batch_size=32,
        learning_rate=1e-3,
        backbone="resnet34",
        params=None,
    ):
        super(SSD300Model, self).__init__(
            "ssd300", 300, batch_size, learning_rate, params=params
        )
        ...
        mlperf.logger.log(key=mlperf.tags.LOC_CONF_OUT_CHANNELS, value=self.out_chan)
        ...
        mlperf.logger.log(key=mlperf.tags.NUM_DEFAULTS_PER_CELL, value=self.num_dboxes)
    ...
    def _localization_loss(self, pred_loc, gt_loc, gt_label, num_matched_boxes):
        ...
        smooth_l1 = tf.reduce_sum(
            tf.losses.huber_loss(gt_loc, pred_loc, reduction=tf.losses.Reduction.NONE),
            axis=2,
        )
    ...
    def accuracy_function(self, inputs, logits):
        ...
        _, gt_boxes, gt_classes, source_id, raw_shape = (
            inputs  # pylint: disable=unused-variable
        )
        ...
        return {
            ...
            (
                constants.UNREDUCED_ACCURACY_OP_PREFIX + ssd_constants.PRED_SCORES
            ): pred_scores,
            ...
            (
                constants.UNREDUCED_ACCURACY_OP_PREFIX + ssd_constants.SOURCE_ID
            ): source_id,
            (
                constants.UNREDUCED_ACCURACY_OP_PREFIX + ssd_constants.RAW_SHAPE
            ): raw_shape,
        }
    ...
    def postprocess(self, results):
        ...
        if len(self.predictions) >= ssd_constants.COCO_NUM_VAL_IMAGES:
            log_fn(
                "Got results for all {:d} eval examples. Calculate mAP...".format(
                    ssd_constants.COCO_NUM_VAL_IMAGES
                )
            )

            annotation_file = os.path.join(
                self.params.data_dir, ssd_constants.ANNOTATION_FILE
            )
            ...
            if self.params.collect_eval_results_async:
                ...
                if not self.async_eval_process:
                    ...
                    self.async_eval_process = multiprocessing.Process(
                        ...
                        args=(
                            self.async_eval_predictions_queue,
                ...
                return {"top_1_accuracy": 0, "top_5_accuracy": 0.0}

            eval_results = coco_metric.compute_map(decoded_preds, annotation_file)
            self.eval_coco_ap = eval_results["COCO/AP"]
            ret = {"top_1_accuracy": self.eval_coco_ap, "top_5_accuracy": 0.0}
    ...
    def reached_target(self):
        return (
            ...
            and self.eval_coco_ap >= self.params.stop_at_top_1_accuracy
        )
