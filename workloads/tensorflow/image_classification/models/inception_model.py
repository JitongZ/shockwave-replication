...
class Inceptionv3Model(model.CNNModel):
    ...
    def add_inference(self, cnn):
        ...
        def inception_v3_d(cnn):
            cols = [
                [("conv", 192, 1, 1), ("conv", 320, 3, 3, 2, 2, "VALID")],
                [
                    ("conv", 192, 1, 1),
                    ("conv", 192, 1, 7),
                    ("conv", 192, 7, 1),
...
def inception_v4_sb(cnn):
    cols = [
        [("conv", 64, 1, 1), ("conv", 96, 3, 3, 1, 1, "VALID")],
        [
            ("conv", 64, 1, 1),
            ("conv", 64, 7, 1),
            ("conv", 64, 1, 7),
...
def inception_v4_sc(cnn):
    cols = [[("conv", 192, 3, 3, 2, 2, "VALID")], [("mpool", 3, 3, 2, 2, "VALID")]]
...
class Inceptionv4Model(model.CNNModel):
    ...
    def add_inference(self, cnn):
        def inception_v4_a(cnn):
            cols = [
                [("apool", 3, 3, 1, 1, "SAME"), ("conv", 96, 1, 1)],
                [("conv", 96, 1, 1)],
        ...
        def inception_v4_b(cnn):
            cols = [
                [("apool", 3, 3, 1, 1, "SAME"), ("conv", 128, 1, 1)],
                [("conv", 384, 1, 1)],
        ...
        def inception_v4_c(cnn):
            cols = [
                [("apool", 3, 3, 1, 1, "SAME"), ("conv", 256, 1, 1)],
                [("conv", 256, 1, 1)],
