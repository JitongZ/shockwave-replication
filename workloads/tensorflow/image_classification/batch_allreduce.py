...
class BatchAllReduceAlgorithm(object):
    ...
    def batch_all_reduce(
        ...
        for packer, device_tensors in zip(packers, all_device_tensors):
            ...
            def pack_single_device_tensors(
                packer=packer, device_tensors=device_tensors
            ):
            ...
            with tf.device(device_tensors[0].device):
                if xla_compile:
                    ...
                    if defer_tensors:
                        packed_tensors, put_ops, warmup_ops = (
                            defer_single_device_tensors(packed_tensors)
                        )
                        all_device_put_ops.append(put_ops)
                        all_device_warmup_ops.append(warmup_ops)
                else:
        ...
        for packer, device_tensors in zip(packers, all_device_tensors):
            ...
            def unpack_single_device_tensors(
                packer=packer, device_tensors=device_tensors
            ):
...
class _TensorPacker(object):
    ...
    def undo_maybe_concat_tensors(self, concatenated_tensor):
        ...
        tensors_with_sizes = tf.split(concatenated_tensor, self._orig_sizes)
        tensors_with_shapes = [
