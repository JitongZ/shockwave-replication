...
class ConvNetBuilder(object):
    ...
    def get_custom_getter(self):
        ...
        def inner_custom_getter(getter, *args, **kwargs):
            ...
            if not (
                requested_dtype == tf.float32 and self.variable_dtype == tf.float16
            ):
    ...
    def _conv2d_impl(
        ...
        if self.use_tf_layers:
            ...
            if self.data_format == "NHWC":
                ...
                strides = [1, 1] + strides
            return tf.nn.conv2d(
                input_layer, weights, strides, padding, data_format=self.data_format
            )
    ...
    def _pool(
        ...
        if pool_name == "mpool":
            mlperf.logger.log_max_pool(input_tensor=input_layer, output_tensor=pool)
        self.top_layer = pool
        return pool
    ...
    def apool(
        ...
        return self._pool(
            "apool",
            pooling_layers.average_pooling2d,
            k_height,
            k_width,
            d_height,
            d_width,
    ...
    def affine(
        ...
        with tf.variable_scope(name):
            ...
            mlperf.logger.log(key=mlperf.tags.MODEL_HP_DENSE, value=num_out_channels)
        ...
        with tf.variable_scope(name):
            ...
            if self.use_tf_layers:
                dropout = core_layers.dropout(
                    input_layer, 1.0 - keep_prob, training=self.phase_train
                )
            else:
