...
flags.DEFINE_boolean(
    "forward_only", False, "whether use forward-only or training for benchmarking"
)
flags.DEFINE_boolean(
...
flags.DEFINE_boolean(
    ...
    False,
    "whether to calculate and print training accuracy during " "training",
)
...
flags.DEFINE_integer(
    "batch_group_size",
    1,
    "number of groups of batches processed in the image " "producer.",
)
flags.DEFINE_integer(
    "num_batches",
    None,
...
flags.DEFINE_integer(
    "num_eval_batches",
    None,
    "number of eval batches to run, excluding warmup. " "Defaults to --num_batches",
)
flags.DEFINE_float(
    "num_epochs",
    None,
...
flags.DEFINE_float(
    "num_eval_epochs",
    None,
    "number of eval epochs to run, excluding warmup. " "Defaults to --num_epochs",
)
flags.DEFINE_float(
...
flags.DEFINE_integer(
    "display_every", 10, "Number of local steps after which progress is printed " "out"
)
flags.DEFINE_float(
    "display_perf_ewma",
    None,
...
flags.DEFINE_boolean("use_datasets", True, "Enable use of datasets for input pipeline")
flags.DEFINE_string(
    "input_preprocessor",
    "default",
...
flags.DEFINE_enum(
    "network_topology",
    constants.NetworkTopology.DGX1,
...
flags.DEFINE_enum(
    "optimizer", "sgd", ("momentum", "sgd", "rmsprop", "adam"), "Optimizer to use"
)
flags.DEFINE_float("init_learning_rate", None, "Initial learning rate for training.")
flags.DEFINE_string(
...
flags.DEFINE_float(
    "gradient_clip", None, "Gradient clipping magnitude. Disabled by default."
)
...
flags.DEFINE_boolean(
    "use_python32_barrier", False, "When on, use threading.Barrier at Python 3.2."
)
...
flags.DEFINE_integer(
    "datasets_prefetch_buffer_size", 1, "Prefetching op buffer size per compute device."
)
flags.DEFINE_integer(
...
flags.DEFINE_integer(
    ...
    None,
    "The number of input elements to fetch before they are " "needed for interleaving.",
)
flags.DEFINE_float("max_duration", None, "The maximum training duration in minutes")

flags.DEFINE_boolean(
    "use_multi_device_iterator",
    True,
...
flags.DEFINE_boolean(
    "winograd_nonfused", True, "Enable/disable using the Winograd non-fused algorithms."
)
flags.DEFINE_boolean(
...
flags.DEFINE_boolean(
    "allow_growth", None, "whether to enable allow_growth in GPU_Options"
)
flags.DEFINE_boolean("xla", False, "whether to enable XLA auto-jit compilation")
flags.DEFINE_boolean(
    "xla_compile",
    False,
    "Enable xla to compile the graph. Uncompilable ops will " "result in fatal errors.",
)
flags.DEFINE_boolean(
    "fuse_decode_and_crop", True, "Fuse decode_and_crop for image preprocessing."
)
flags.DEFINE_boolean(
    "distort_color_in_yiq", True, "Distort color of input images in YIQ space."
)
flags.DEFINE_boolean(
    "enable_optimizations", True, "Whether to enable grappler and other optimizations."
)
flags.DEFINE_string(
    "rewriter_config",
    None,
    "Config for graph optimizers, described as a " "RewriterConfig proto buffer.",
)
flags.DEFINE_enum(
    "loss_type_to_report",
    "total_loss",
...
flags.DEFINE_integer("kmp_settings", 1, "If set to 1, MKL settings will be printed.")
...
flags.DEFINE_integer("max_ckpts_to_keep", 5, "Max number of checkpoints to keep.")
flags.DEFINE_string(
    "train_dir",
    None,
...
def make_params(**kwargs):
    ...
    default_kwargs = {
        name: flags.param_specs[name].default_value for name in flags.param_specs
    }
...
def make_params_from_flags():
    ...
    # flags.param_specs.
    flag_values = {
        name: getattr(absl_flags.FLAGS, name) for name in flags.param_specs.keys()
    }
...
def remove_param_fields(params, fields_to_remove):
    ...
    params_dict = {k: v for k, v in params_dict.items() if k not in fields_to_remove}
...
def get_num_batches_and_epochs(params, batch_size, num_examples_per_epoch):
    ...
    if params.num_batches and params.num_epochs:
        raise ValueError(
            "At most one of --num_batches and --num_epochs may be " "specified."
        )
    if params.num_epochs:
        num_batches = (
            int(params.num_epochs * num_examples_per_epoch + batch_size - 1)
            // batch_size
        )
    else:
...
def get_piecewise_learning_rate(
    piecewise_learning_rate_schedule, global_step, num_batches_per_epoch
):
...
def get_learning_rate(params, global_step, num_examples_per_epoch, model, batch_size):
    ...
    with tf.name_scope("learning_rate"):
        ...
        if params.piecewise_learning_rate_schedule:
            ...
            if (
                ...
                if params.minimum_learning_rate != 0.0:
                    learning_rate = tf.maximum(
                        learning_rate, params.minimum_learning_rate
                    )
        else:
...
def get_optimizer(params, learning_rate):
    ...
    if params.optimizer == "momentum":
        mlperf.logger.log(key=mlperf.tags.OPT_NAME, value=mlperf.tags.SGD_WITH_MOMENTUM)
        ...
        opt = tf.train.AdamOptimizer(
            learning_rate, params.adam_beta1, params.adam_beta2, params.adam_epsilon
        )
    else:
...
class BenchmarkCNN(object):
    ...
    def __init__(self, params, dataset=None, model=None):
        ...
        autotune_threshold = (
            self.params.autotune_threshold if (self.params.autotune_threshold) else 1
        )
        min_autotune_warmup = 5 * autotune_threshold * autotune_threshold
        self.num_warmup_batches = (
            self.params.num_warmup_batches
            if (self.params.num_warmup_batches is not None)
        ...
        if (
            ...
            and self.params.data_format == "NCHW"
            and not self.params.mkl
        ):
            raise ValueError("device=cpu requires that data_format=NHWC")

        if (
            self.params.num_epochs_per_decay or self.params.learning_rate_decay_factor
        ) and not (
            self.params.init_learning_rate is not None
        ...
        if self.params.minimum_learning_rate and not (
            self.params.init_learning_rate is not None
            and self.params.num_epochs_per_decay
            and self.params.learning_rate_decay_factor
        ):
        ...
        if (
            self.params.use_fp16
            and self.params.fp16_vars
        ...
        if self.params.use_fp16 and self.params.fp16_enable_auto_loss_scale:
            ...
            if self.params.variable_update not in (
                "parameter_server",
                "replicated",
                "independent",
            ):
            ...
            if self.params.staged_vars:
                raise ValueError(
                    "Automatic loss scaling is not supported with" "staged_vars."
                )

        if (
            self.params.debugger is not None
            and self.params.debugger != "cli"
        ...
        if self.params.hierarchical_copy and self.params.num_gpus <= 1:
            raise ValueError(
                "--hierarchical_copy requires --num_gpus to be greater " "than 1"
            )
        ...
        use_controller = (
            self.params.variable_update == "distributed_all_reduce" and self.job_name
        )
        ...
        self.distributed_collective = (
            self.params.variable_update == "collective_all_reduce" and self.job_name
        )
        ...
        if self.num_workers > 1 and self.params.all_reduce_spec == "nccl":
            raise ValueError(
                "--all_reduce_spec=nccl is invalid in a " "multi-worker job"
            )
        ...
        if self.params.eval_during_training_at_specified_steps:
            try:
                self.eval_during_training_at_specified_steps = set(
                    map(int, self.params.eval_during_training_at_specified_steps)
                )
            except ValueError:
                raise ValueError(
        ...
        if self.params.eval_during_training_at_specified_epochs:
            try:
                n_epochs = list(
                    map(float, self.params.eval_during_training_at_specified_epochs)
                )
                offset = n_epochs[0] - 1
        ...
        self.datasets_use_prefetch = (
            ...
            # CPU.
            self.params.device.lower() != "cpu"
            and self.input_preprocessor
    ...
    def _config_benchmark_logger(self):
        ...
        if self.params.benchmark_log_dir is not None:
            try:
                from official.utils.logs import (
                    logger as models_logger,
    ...
    def reset_devices_for_task(self, task_num, is_local=False):
        ...
        worker_prefix = (
            "/job:localhost" if is_local else "/job:worker/replica:0/task:%s" % task_num
        )
        self.cpu_device = "%s/cpu:0" % worker_prefix
        self.raw_devices = [
            "%s/%s:%i" % (worker_prefix, self.params.device, i)
    ...
    def print_info(self):
        ...
        log_fn(
            "             %s per device" % (self.batch_size // len(self.raw_devices))
        )
        if self.batch_group_size > 1:
            log_fn(
                "             %d batches per prepocessing group" % self.batch_group_size
            )
    ...
    def _run_eval(self):
        ...
        summary_writer = tf.summary.FileWriter(
            self.params.eval_dir, tf.get_default_graph()
        )
        target = ""
    ...
    def _initialize_eval_graph(
        self, enqueue_ops, input_producer_op, local_var_init_op_group, sess
    ):
    ...
    def _eval_once(
        self, sess, summary_writer, fetches, summary_op, image_producer, global_step
    ):
        ...
        with self._do_eval():
            ...
            for step in xrange(self.num_batches):
                if (
                    ...
                    and (step + 1) % self.params.save_summaries_steps == 0
                ):
            ...
            for result_key, result_value in results.items():
                if result_key.startswith(constants.SIMPLE_VALUE_RESULT_PREFIX):
                    ...
                    summary.value.add(
                        tag="eval/" + result_key[prefix_len:], simple_value=result_value
                    )
            if summary_writer:
            ...
            mlperf.logger.log(
                key=mlperf.tags.EVAL_SIZE, value=self.num_batches * self.batch_size
            )
    ...
    def _build_graph(self):
        ...
        if (
            (not self.single_session)
            and (not self.distributed_collective)
            and self.job_name
            and self.params.cross_replica_sync
        ):
        ...
        if (
            ...
            variable_manager_init_ops.append(
                self.add_sync_queues_and_barrier(
                    "init_ops_end_", variable_manager_init_ops
                )
            )
        local_var_init_op_group = tf.group(
            *variable_manager_init_ops, name="local_var_init_op_group"
        )
    ...
    def _benchmark_graph(self, graph_info, eval_graph_info):
        ...
        if (
            is_chief
            and self.params.summary_verbosity
            and self.params.train_dir
            and self.params.save_summaries_steps > 0
        ):
            summary_writer = tf.summary.FileWriter(
                self.params.train_dir, tf.get_default_graph()
            )
        ...
        sv = tf.train.Supervisor(
            ...
            is_chief=is_chief
            or (
                self.params.variable_update == "horovod" or self.distributed_collective
            ),
        ...
        if self.graph_file is not None:
            ...
            log_fn(
                ...
                % ("text" if as_text else "binary", self.graph_file)  # pyformat break
            )
        ...
        start_standard_services = (
            self.params.train_dir or self.dataset.queue_runner_required()
        )
    ...
    def benchmark_with_session(
        ...
        if self.single_session:
            ...
            end_local_step = self.num_batches - (
                self.init_global_step // self.num_workers
            )
        ...
        while not done_fn():
            ...
            if (
                summary_writer
                and (local_step + 1) % self.params.save_summaries_steps == 0
            ):
                fetch_summary = graph_info.summary_op
            else:
                fetch_summary = None
            collective_graph_key = (
                7 if (self.params.variable_update == "collective_all_reduce") else 0
            )
            ...
            local_step += 1
            if (
                self.params.save_model_steps
                and local_step % self.params.save_model_steps == 0
                and local_step > 0
                and is_chief
            ):
                supervisor.saver.save(
                    sess, supervisor.save_path, supervisor.global_step
                )
            if (
                eval_graph_info
                and local_step > 0
                ...
                log_fn(
                    "Running evaluation at global_step {}".format(python_global_step)
                )
                accuracy_at_1, _ = self._eval_once(
                    sess,
                    summary_writer,
                    eval_graph_info.fetches,
                    eval_graph_info.summary_op,
                    eval_image_producer,
                    python_global_step,
                )
                last_eval_step = local_step
        ...
        if not global_step_watcher:
            ...
            images_per_sec = (
                self.num_workers * local_step * self.batch_size / elapsed_time
            )
            ...
            average_wall_time = (
                elapsed_time * self.num_workers / num_steps if num_steps > 0 else 0
            )
        ...
        if eval_graph_info and not skip_final_eval:
            log_fn(
                "Running final evaluation at global_step {}".format(python_global_step)
            )
            accuracy_at_1, _ = self._eval_once(
                sess,
                summary_writer,
        ...
        success = bool(
            ...
            or (
                accuracy_at_1
                and self.params.stop_at_top_1_accuracy
                and accuracy_at_1 >= self.params.stop_at_top_1_accuracy
            )
        )
    ...
    def _preprocess_graph(self, graph, graph_info):
        ...
        if self.params.trt_mode:
            ...
            name_to_input_name, _, _ = graph_util_impl._extract_graph_summary(graphdef)
    ...
    def _build_input_processing(self, shift_ratio=0):
        ...
        if self.datasets_use_prefetch:
            if self.params.use_multi_device_iterator:
                multi_device_iterator = input_preprocessor.build_multi_device_iterator(
                    self.batch_size,
                    len(self.devices),
                    self.cpu_device,
                    self.params,
            ...
            function_buffering_resources = (
                input_preprocessor.build_prefetch_input_processing(
                    ...
                    self.params,
                    self.devices,
                    self.model.get_input_data_types(subset),
                    self.dataset,
                    self._doing_eval,
                )
            )
        ...
        with tf.device(self.cpu_device):
            ...
            input_list = input_preprocessor.minibatch(
                self.dataset, subset=subset, params=self.params, shift_ratio=shift_ratio
            )
    ...
    def _maybe_initialize_fp16(self):
        ...
        if self.params.use_fp16 and not self._doing_eval:
            init_loss_scale_val = float(
                self.params.fp16_loss_scale or self.model.get_fp16_loss_scale()
            )
            self.loss_scale = None
            self.loss_scale_normal_steps = None
    ...
    def _build_model(self):
        ...
        if not self.datasets_use_prefetch:
            ...
            enqueue_ops.append(
                tf.group(*gpu_compute_stage_ops, name="gpu_compute_stage_ops_group")
            )
        ...
        if (
            self.mode == constants.BenchmarkMode.TRAIN_AND_EVAL
            ...
            mlperf.logger.log(
                key=mlperf.tags.INPUT_BN_SPAN,
                value=self.batch_size // len(self.raw_devices),
            )

        fetches = self._build_fetches(
            global_step,
            all_logits,
            losses,
            device_grads,
            ...
            with tf.device(device):
                ...
                if gradient_clip is not None:
                    with tf.name_scope("clip_gradients"):
                        clipped_grads = [
                            (
                                tf.clip_by_value(grad, -gradient_clip, +gradient_clip),
                                var,
                            )
        ...
        for task_num in range(self.num_workers):
            ...
            for rel_device_num in range(len(self.devices)):
                ...
                with self.variable_mgr.create_outer_variable_scope(
                    ...
                    "task_%i_tower_%i" % (task_num, rel_device_num)
                ) as name_scope:
    ...
    def add_forward_pass_and_gradients(
        ...
        if self.datasets_use_prefetch:
            ...
            if input_processing_info.multi_device_iterator_input:
                input_data = input_processing_info.multi_device_iterator_input[
                    rel_device_num
                ]
            ...
            if function_buffering_resource is None and input_data is None:
                raise ValueError(
                    "Both function_buffering_resource and input_data "
        ...
        def forward_pass_and_gradients():
            ...
            if weight_decay is not None and weight_decay != 0.0 and l2_loss is not None:
                mlperf.logger.log(
                    key=mlperf.tags.MODEL_L2_REGULARIZATION, value=weight_decay
                )
                total_loss += len(self.devices) * weight_decay * l2_loss

            aggmeth = tf.AggregationMethod.DEFAULT
            scaled_loss = (
                total_loss if self.loss_scale is None else total_loss * self.loss_scale
            )
            ...
            if self.loss_scale is not None:
                ...
                grads = [
                    grad * tf.cast(1.0 / self.loss_scale, grad.dtype) for grad in grads
                ]
            ...
            if self.params.variable_update == "horovod":
                ...
                grads = [
                    hvd.allreduce(grad, average=False, device_dense=horovod_device)
                    for grad in grads
                ]
        ...
        def unpack_forward_pass_and_gradients_output(forward_pass_and_grad_outputs):
            ...
            loss = (
                ...
                if forward_pass_and_grad_outputs
                else None
            )
            grads = (
...
def _is_mkl_flag_absent(mkl_flag):
    return not (
        absl_flags.FLAGS.is_parsed()
        and mkl_flag in absl_flags.FLAGS
...
def _set_environ_vars(params):
    ...
    if params.device.lower() == "gpu":
        ...
        if params.datasets_use_prefetch and params.datasets_num_private_threads is None:
