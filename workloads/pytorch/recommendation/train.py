...
parser.add_argument(
    "-n", "--num_epochs", required=True, type=int, help="Number of epochs to run for"
)
parser.add_argument(
...
parser.add_argument("-b", "--batch_size", default=2048, type=int, help="Batch size")
parser.add_argument(
    "--checkpoint_dir",
    type=str,
...
val_te_matrix, _, _ = dataframe_to_csr_matrix(
    val_te_df, item_id_map=item_id_map, user_id_map=user_id_map, **common_params
)
...
model = DynamicAutoencoder(
    hidden_layers=[200], activation_type="tanh", noise_prob=0.5, sparse=False
)
