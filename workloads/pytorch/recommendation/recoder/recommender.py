...
class SimilarityRecommender(Recommender):
    ...
    def __init__(
        self, embeddings_index: EmbeddingsIndex, num_recommendations, n=1, scale=1
    ):
    ...
    def __recommend_single(self, user_hist):
        ...
        items_pool = [
            self.embeddings_index.get_nns_by_id(item_id, self.n)
        ...
        if len(items_scores) > self.num_recommendations:
            top_ind_not_sorted = np.argpartition(
                -items_scores, self.num_recommendations
            )
    ...
    def __compute_scores(self, items_pool, user_items):
        pool_embeddings = np.array(
            [self.embeddings_index.get_embedding(item_id) for item_id in items_pool]
        )
        user_embeddings = np.array(
            [self.embeddings_index.get_embedding(item_id) for item_id in user_items]
        )
        ...
        scores = np.dot(
            pool_embeddings, np.transpose(user_embeddings)
        )  # range: -1 to 1
    ...
    def recommend(self, users_hist):
        recommendations = [
            self.__recommend_single(user_hist) for user_hist in users_hist
        ]
...
class InferenceRecommender(Recommender):
    ...
    def __init__(self, model, num_recommendations):
        self.model = model
        self.num_recommendations = num_recommendations
