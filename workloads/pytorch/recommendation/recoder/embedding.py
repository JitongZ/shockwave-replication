...
class AnnoyEmbeddingsIndex(EmbeddingsIndex):
    ...
    def get_nns_by_id(self, embedding_id, n):
        nearest_indices = self.index.get_nns_by_item(
            self.id_map[embedding_id],
            n,
        ...
        if not self.include_distances:
            ...
            nearest_ids = dict(
                zip(
                    [self.inverse_id_map[ind] for ind in nearest_indices[0]],
                    nearest_indices[1],
                )
            )
    ...
    def get_nns_by_embedding(self, embedding, n):
        nearest_indices = self.index.get_nns_by_vector(
            embedding,
            n,
            search_k=self.search_k,
            include_distances=self.include_distances,
...
class MemCacheEmbeddingsIndex(EmbeddingsIndex):
    ...
    def get_nns_by_id(self, embedding_id, n):
        if embedding_id not in self.__nns_cache:
            self.__nns_cache[embedding_id] = self.embedding_index.get_nns_by_id(
                embedding_id, n
            )
