...
class RecommendationDataset(Dataset):
    ...
    def __init__(self, interactions_matrix, target_interactions_matrix=None):
        ...
        self.target_interactions_matrix = (
    ...
    def __getitem__(self, index):
        ...
        if self.target_interactions_matrix is None:
            return (
                UsersInteractions(
                    users=users, interactions_matrix=extracted_sparse_matrix
                ),
                None,
            )
        else:
            ...
            return UsersInteractions(
                ...
                users=users, interactions_matrix=extracted_target_sparse_matrix
            )
        ...
        if sputils.issequence(index) and len(index) > CSR_MATRIX_INDEX_SIZE_LIMIT:
            ...
            for offset in range(0, len(index), CSR_MATRIX_INDEX_SIZE_LIMIT):
                sparse_matrix_slices.append(
                    sparse_matrix[index[offset : offset + CSR_MATRIX_INDEX_SIZE_LIMIT]]
                )
...
class RecommendationDataLoader:
    ...
    def __init__(
        ...
        assert (
            self.num_sampling_users >= batch_size
        ), "num_sampling_users should be at least equal to the batch_size"

        self.batch_collator = BatchCollator(
            batch_size=self.batch_size, negative_sampling=self.negative_sampling
        )
...
class Batch:
    ...
    def __init__(self, users, items, indices, values, size):
        self.users = users
        self.items = items
...
class BatchCollator:
    ...
    def collate(self, users_interactions):
        ...
        current_ind = 0
        for offset in range(
            0, users_interactions.interactions_matrix.shape[0], self.batch_size
        ):
            slice_sparse_matrix = users_interactions.interactions_matrix[
