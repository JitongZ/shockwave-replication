...
class Recoder(object):
    ...
    def __init_optimizer(self, lr, weight_decay):
        ...
        if (
            self.__sparse_optimizer_state_dict is not None
    ...
    def __init_training(self, train_dataset, lr, weight_decay):
    ...
    def train(
        ...
        assert (
            num_sampling_users >= batch_size and num_sampling_users % batch_size == 0
        ), "number of sampling users should be a multiple of the batch size"

        self.__init_training(
            train_dataset=train_dataset, lr=lr, weight_decay=weight_decay
        )

        train_dataloader = RecommendationDataLoader(
            train_dataset,
        ...
        if val_dataset is not None:
            val_dataloader = RecommendationDataLoader(
                val_dataset,
                batch_size=batch_size,
                negative_sampling=negative_sampling,
                num_sampling_users=num_sampling_users,
        ...
        if self._enable_gavel_iterator:
            train_dataloader = GavelIterator(
                train_dataloader,
                self._gavel_dir,
            ...
            try:
                ...
                if self._enable_gavel_iterator:
                    self.init_from_model_file(
                        model_checkpoint_prefix, local_rank, train_dataloader
                    )
                else:
        ...
        for epoch in range(current_epoch, num_epochs + 1):
            ...
            for batch_itr, (input, target) in iterator:
                ...
                progress_bar.set_postfix(
                    loss=np.mean(aggregated_losses[-1]),
                    num_items=num_items,
                    refresh=False,
                )
                progress_bar.update()
            ...
            if model_checkpoint_prefix and (
                (checkpoint_freq > 0 and epoch % checkpoint_freq == 0)
                or epoch == num_epochs
            ):
                if self._enable_gavel_iterator:
                    self.save_state(model_checkpoint_prefix, train_dataloader)
    ...
    def __compute_loss(self, input, target):
        ...
        input_dense = (
            torch.sparse.FloatTensor(input.indices, input.values, input.size)
        ...
        if target is not None:
            ...
            target_dense = (
                torch.sparse.FloatTensor(target.indices, target.values, target.size)
                .to(device=self.device)
                .to_dense()
            )
            if target_items is not None:
    ...
    def predict(self, users_interactions, return_input=False):
        ...
        batch_collator = BatchCollator(
            batch_size=len(users_interactions.users), negative_sampling=False
        )

        input = batch_collator.collate(users_interactions)
        batch = input[0]
        input_dense = (
            torch.sparse.FloatTensor(batch.indices, batch.values, batch.size)
            .to(device=self.device)
            .to_dense()
        )
    ...
    def _evaluate(
        self, eval_dataset, num_recommendations, metrics, batch_size=1, num_users=None
    ):
        if self.model is None:
            raise Exception("Model not initialized")
