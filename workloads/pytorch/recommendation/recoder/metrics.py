...
def average_precision(x, y, k, normalize=True):
    ...
    precision_drecall = np.multiply(
        precision, x_in_y
    )  # precision * delta_recall at every position
...
class RecommenderEvaluator(object):
    ...
    def evaluate(self, eval_dataset, batch_size=1, num_users=None, num_workers=0):
        """
        ...
        dataloader = RecommendationDataLoader(
            eval_dataset, batch_size=batch_size, collate_fn=lambda _: _
        )
        ...
        if num_workers > 0:
            ...
            workers = [
                Process(
                    target=evaluate,
                    args=(input_queue, results_queues[p_idx], self.metrics),
                )
        ...
        for batch in dataloader:
            ...
            relevant_items = [
                target.interactions_matrix[i].nonzero()[1]
                for i in range(len(target.users))
            ]
