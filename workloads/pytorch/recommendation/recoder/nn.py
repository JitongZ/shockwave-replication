...
class DynamicAutoencoder(FactorizationModel):
    ...
    def __create_encoding_layers(self):
        self.en_embedding_layer = nn.Embedding(
            self.num_embeddings, self.hidden_layers[0], sparse=self.sparse
        )

        self.__en_linear_embedding_layer = LinearEmbedding(
            self.en_embedding_layer, input_based=True
        )
        self.encoding_layers = nn.Sequential(
            *self.__create_coding_layers(self.hidden_layers)
        )
    ...
    def __create_decoding_layers(self):
        _decoding_layers = self.__create_coding_layers(
        ...
        self.__de_linear_embedding_layer = LinearEmbedding(
            self.de_embedding_layer, input_based=False
        )
...
class LinearEmbedding(nn.Module):
    ...
    def __init__(self, embedding_layer: nn.Embedding, input_based=True, bias=True):
        ...
        self.in_features = (
            ...
            if input_based
            else embedding_layer.embedding_dim
        )
        self.out_features = (
            embedding_layer.embedding_dim
            if input_based
            else embedding_layer.num_embeddings
        )
        if bias:
...
class MatrixFactorization(FactorizationModel):
    ...
    def __init__(
        self, embedding_size, activation_type="none", dropout_prob=0, sparse=False
    ):
        super().__init__()
    ...
    def init_model(self, num_items=None, num_users=None):
        ...
        self.user_embedding_layer = nn.Embedding(
            self.num_users, self.embedding_size, sparse=self.sparse
        )
        self.item_embedding_layer = nn.Embedding(
            self.num_items, self.embedding_size, sparse=self.sparse
        )
