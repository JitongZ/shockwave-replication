...
@pytest.mark.parametrize(
    "sparse,exp_recall_20,exp_recall_50,exp_ndcg_100",
    [
...
def test_model(sparse, exp_recall_20, exp_recall_50, exp_ndcg_100):
    ...
    train_matrix, item_id_map, user_id_map = dataframe_to_csr_matrix(
        train_df, user_col="uid", item_col="sid", inter_col="watched"
    )

    val_matrix, _, _ = dataframe_to_csr_matrix(
        val_df,
        user_col="uid",
        item_col="sid",
        inter_col="watched",
    ...
    model = DynamicAutoencoder(
        hidden_layers=[200], activation_type="tanh", noise_prob=0.5, sparse=sparse
    )
    trainer = Recoder(
        model=model, use_cuda=use_cuda, optimizer_type="adam", loss="logloss"
    )
    ...
    results = trainer._evaluate(
        ...
        metrics=[recall_20, recall_50, ndcg_100],
        batch_size=500,
    )
