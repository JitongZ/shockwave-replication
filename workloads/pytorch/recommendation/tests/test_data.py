...
def test_RecommendationDataset_target(input_dataframe, target_dataframe):
    common_users = input_dataframe.merge(
        target_dataframe, how="inner", on="user"
    ).user.unique()
    common_items = input_dataframe.merge(
        target_dataframe, how="inner", on="item"
    ).item.unique()
    ...
    assert (
        input_interactions.interactions_matrix.getnnz() > 0
        and target_interactions.interactions_matrix.getnnz() > 0
    )


@pytest.mark.parametrize("batch_size,num_sampling_users", [(5, 0), (5, 10)])
def test_RecommendationDataLoader(
    input_dataframe, target_dataframe, batch_size, num_sampling_users
):
    common_users = input_dataframe.merge(
        target_dataframe, how="inner", on="user"
    ).user.unique()
    ...
    for batch_idx, (input, target) in enumerate(dataloader, 1):
        input_idx, input_val, input_size, input_items = (
            input.indices,
            input.values,
            input.size,
            input.items,
        )
        input_dense = torch.sparse.FloatTensor(
            input_idx, input_val, input_size
        ).to_dense()

        target_idx, target_val, target_size, target_words = (
            target.indices,
            target.values,
            target.size,
            target.items,
        )
        target_dense = torch.sparse.FloatTensor(
            target_idx, target_val, target_size
        ).to_dense()

        assert target is not None

        assert (
            input_dense.size(0) == batch_size
            or batch_idx == len(dataloader)
            and input_dense.size(0) == len(dataset) % batch_size
        )
        assert input_dense.size(1) == len(input_items)


@pytest.mark.parametrize("batch_size", [1, 2, 5, 10, 13])
def test_BatchCollator(input_dataframe, batch_size):
    ...
    batch_collator = BatchCollator(batch_size=batch_size, negative_sampling=True)
    ...
    for batch in batches:
        input_idx, input_val, input_size, input_words = (
            batch.indices,
            batch.values,
            batch.size,
            batch.items,
        ...
        item_idx_map = {
            item_id: item_idx for item_idx, item_id in enumerate(input_words.tolist())
        }

        for user_idx in range(len(batch_users)):
            for item_id, val in zip(
                batch_sparse_matrix[user_idx].nonzero()[1],
