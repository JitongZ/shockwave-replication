...
def train_epoch(
    ...
    for batch in tqdm(
        ...
        if cumulative_step is not None:
            ...
            if (
                throughput_estimation_interval is not None
                and cumulative_step % throughput_estimation_interval == 0
            ):
                print(
                    "[THROUGHPUT_ESTIMATION]\t%s\t%d" % (time.time(), cumulative_step)
                )
...
def train(model, training_data, validation_data, optimizer, device, opt):
    ...
    if opt.step is not None:
        opt.epoch = math.ceil(float(opt.step) * opt.batch_size / len(training_data))

    for epoch_i in range(start_epoch, opt.epoch):
        print("[ Epoch", epoch_i, "]")

        start = time.time()
        train_loss, train_accu, cumulative_step, cumulative_time, done = train_epoch(
            model,
            training_data,
            optimizer,
            device,
...
def main():
    ...
    parser.add_argument(
        "--checkpoint_dir", type=str, default="/lfs/1/keshav2/checkpoints/transformer"
    )
    ...
    if opt.distributed:
        transformer = DDP(
            transformer, device_ids=[opt.local_rank], output_device=opt.local_rank
        )

    if opt.enable_gavel_iterator:
        training_data = GavelIterator(
            training_data, opt.checkpoint_dir, load_checkpoint, save_checkpoint
        )
