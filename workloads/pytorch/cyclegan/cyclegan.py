...
parser.add_argument(
    "--checkpoint_dir",
    type=str,
    default="/lfs/1/keshav2/checkpoints/cyclegan",
    help="Checkpoint dir",
)
parser.add_argument(
    "--n_steps", type=int, default=None, help="number of steps of training"
)
parser.add_argument(
    "--n_epochs", type=int, default=None, help="number of epochs of training"
)
parser.add_argument(
    "--dataset_path", type=str, required=True, help="Path to the dataset"
)
parser.add_argument(
    "--dataset_name", type=str, default="monet2photo", help="Name of the dataset"
)
...
val_dataloader = DataLoader(
    ImageDataset(
        opt.dataset_path, transforms_=transforms_, unaligned=True, mode="test"
    ),
    batch_size=5,
    shuffle=True,
    num_workers=1,
...
if opt.enable_gavel_iterator:
    dataloader = GavelIterator(
        dataloader, opt.checkpoint_dir, load_checkpoint, save_checkpoint
    )
...
for epoch in range(start_epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        sys.stdout.write(
            ...
            % (
                ...
                (
                    ...
                    else opt.n_steps - epoch * len(dataloader)
                ),
                loss_D.item(),
                loss_G.item(),
                loss_GAN.item(),
        ...
        if (
            opt.throughput_estimation_interval is not None
            and steps % opt.throughput_estimation_interval == 0
        ):
            print("")
            sys.stdout.flush()
