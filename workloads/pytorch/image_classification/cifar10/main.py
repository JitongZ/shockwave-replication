...
parser.add_argument(
    "--num_epochs", default=None, type=int, help="Number of epochs to train for"
)
parser.add_argument(
    "--num_steps", default=None, type=int, help="Number of steps to train for"
)
parser.add_argument("--batch_size", default=64, type=int, help="Batch size")
parser.add_argument("--lr", default=0.1, type=float, help="learning rate")
parser.add_argument(
    "--checkpoint_dir",
    default="/lfs/1/keshav2/checkpoints/resnet-18",
    type=str,
...
parser.add_argument("--log_interval", type=int, default=100, help="Interval to log")
parser.add_argument(
    "--dist-url",
    default="env://",
    type=str,
    ...
    if os.path.exists(checkpoint_path):
        try:
            checkpoint = torch.load(
                checkpoint_path, map_location="cuda:{}".format(args.local_rank)
            )
...
def train(epoch, cumulative_steps=None, cumulative_time=None):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if cumulative_time is not None:
            ...
            if args.max_duration is not None and cumulative_time > args.max_duration:
                done = True
...
if args.num_steps is not None:
    args.num_epochs = math.ceil(
        float(args.num_steps) * args.batch_size / len(trainloader)
    )

for epoch in range(start_epoch, args.num_epochs):
    (cumulative_steps, cumulative_time, done, finished_epoch) = train(
        epoch, cumulative_steps, cumulative_time
    )
    if args.enable_gavel_iterator:
        if trainloader.done:
            break
