...
parser.add_argument(
    "--num_minibatches", default=None, type=int, help="number of minibatches to run"
)
parser.add_argument(
    "--start-epoch",
    default=0,
...
def main():
    ...
    if args.num_minibatches is not None:
        args.epochs = math.ceil(
            float(args.num_minibatches) * args.batch_size / len(train_loader)
        )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        num_minibatches, elapsed_time, finished_epoch = train(
            train_loader,
            model,
            criterion,
            optimizer,
        ...
        if args.enable_gavel_iterator and train_loader.done:
            ...
            args.num_minibatches is not None
            and total_minibatches >= args.num_minibatches
        ):
            if args.enable_gavel_iterator:
                train_loader.complete()
            break
        elif args.max_duration is not None and total_elapsed_time >= args.max_duration:
            if args.enable_gavel_iterator:
                train_loader.complete()
            break
...
def train(
    ...
    for i, (input, target) in enumerate(train_loader):
        if total_minibatches is not None and i + total_minibatches >= max_minibatches:
            finished_epoch = False
            break
        elif (
            max_duration is not None
            and total_elapsed_time + elapsed_time >= max_duration
        ):
            finished_epoch = False
            break
